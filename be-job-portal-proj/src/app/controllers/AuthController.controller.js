...
class AuthController {
    ...
    if (
      ...
      bcrypt.compare(password, member.password, async (err, result) => {
        if (result) {
          ...
          let token = jwt.sign(payload, process.env.jwtSecretKey, { expiresIn: "3d" });
  ...
  async candidateRegister(req, res) {
    ...
    await bcrypt
      .hash(info.password, proccess.env.BCRYPT_SALT_ROUND)
    ...
    try {
      ...
      await bcrypt
        .hash(info.email, process.env.BCRYPT_SALT_ROUND)
        .then((hashEmail) => {
  ...
  async sendMail(req, res) {
    ...
    if (member) {
      if (member.verifiedAt === null) {
        ...
        const emailTemplate = fs.readFileSync(emailTemplatePath, "utf8");
        await bcrypt.hash(email, process.env.BCRYPT_SALT_ROUND).then((hashEmail) => {
          const emailContent = emailTemplate.replace(
            "{{verificationLink}}",
  ...
  async employerRegister(req, res) {
    ...
    await bcrypt
      .hash(info.password, process.env.BCRYPT_SALT_ROUND)
      .then((hashPassword) => (info = { ...info, password: hashPassword }));
