...
function PostedJob() {
  ...
  const handleConfirmHidden = async (postedJobs) => {
    ...
    await axios.post(`${API_DOMAIN}/api/company/hidden-job/`, {
      jobs: jobIds,
    }, {
  ...
  const handleConfirmEnable = async (postedJobs) => {
    ...
    const jobIds = postedJobs.map(job => job._id);
    await axios.post(`${API_DOMAIN}/api/company/enable-job/`, {
      jobs: jobIds,
    }, {
      withCredentials: true,
    })
  ...
  const getPostedJob = (page, pageSize, hidden = false, pos = "Unknow",) => {
    setLoading(true);
    axios.get(`${API_DOMAIN}/api/company/jobs?hidden=${hidden}&page=${page}&size=${pageSize}`, {
      withCredentials: true,
    })
      .then(res => {
  ...
  const getAllCategories = () => {
    axios.get(`${API_DOMAIN}/api/job-category/all`)
      .then(res => {
        ...
        {jobInfo !== null && jobCategories ? (
          <ModalPostJob
            ...
            apiUpdate={jobInfo.title
              ? `${API_DOMAIN}/api/employer/job/${jobInfo._id}`
              : `${API_DOMAIN}/api/company/post-job`
            }
