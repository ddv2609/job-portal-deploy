...
function PostedJob() {
  ...
  const handleConfirmHidden = async (postedJobs) => {
    ...
    await axios.post(`${API_DOMAIN}/api/admin/posted-job/hidden`, {
      jobs: jobIds,
      adminId: admin.id,
    }, {
  ...
  const handleConfirmEnable = async (postedJobs) => {
    ...
    const jobIds = postedJobs.map(job => job._id);
    await axios.post(`${API_DOMAIN}/api/admin/posted-job/enable`, {
      jobs: jobIds,
    }, {
      withCredentials: true,
    })
  ...
  const handleConfirmDelete = async (postedJobs) => {
    ...
    const jobIds = postedJobs.map(job => job._id);
    await axios.post(`${API_DOMAIN}/api/admin/posted-job/delete`, {
      jobs: jobIds,
    }, {
      withCredentials: true,
    })
  ...
  const getPostedJob = (page, pageSize, hidden = false, pos = "Unknow",) => {
    setLoading(true);
    axios.get(`${API_DOMAIN}/api/admin/list/posted-job?hidden=${hidden}&page=${page}&size=${pageSize}`, {
      withCredentials: true,
    })
      .then(res => {
