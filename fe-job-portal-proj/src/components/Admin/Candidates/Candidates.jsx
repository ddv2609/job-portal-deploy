...
function Candidates() {
  ...
  const handleConfirmHidden = async (candidates) => {
    ...
    await axios.post(`${API_DOMAIN}/api/admin/hidden`, {
  ...
  const handleConfirmEnable = async (candidates) => {
    ...
    await axios.post(`${API_DOMAIN}/api/admin/enable`, {
      members: candidates.map(candidate => ({ mbid: candidate._id, email: candidate.email, })),
    }, {
      withCredentials: true,
    })
  ...
  const handleConfirmVerify = async (candidates) => {
    ...
    await axios.post(`${API_DOMAIN}/api/admin/verify`, {
      members: candidates.map(candidate => ({ mbid: candidate._id, email: candidate.email, })),
    }, {
      withCredentials: true,
    })
  ...
  const handleConfirmDelete = async (candidates) => {
    ...
    await axios.post(`${API_DOMAIN}/api/admin/candidates/delete`, {
      members: candidates.map(candidate => ({ mbid: candidate._id, email: candidate.email, })),
    }, {
      withCredentials: true,
    })
  ...
  const getCandidates = (page, pageSize, hidden = false, pos = "Unknow",) => {
    setLoading(true);
    axios.get(`${API_DOMAIN}/api/admin/list/candidate?hidden=${hidden}&page=${page}&size=${pageSize}`, {
      withCredentials: true,
    })
      .then(res => {
  ...
  return (
    <>
      ...
      {modalData && <ModalUpdate
        ...
        apiUpdate={`${API_DOMAIN}/api/admin/member/info/${modalData._id}`}
      >
        <Col span={24}>
          <Form.Item
